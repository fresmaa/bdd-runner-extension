...
import { FeatureContext, ReportType, RunCommandInput, RunMode, ScenarioContext, ScenarioLocation } from "./types";
...
export function activate(context: vscode.ExtensionContext): void {
  ...
  const generateReport = vscode.commands.registerCommand(
    "bddScenarioRunner.generateReport",
    async () => {
      await generateReportCommand();
    },
  );

  const generatePlaywrightReport = vscode.commands.registerCommand(
    "bddScenarioRunner.generatePlaywrightReport",
    async () => {
      runReportByType("playwright");
    },
  );

  const generateAllureReport = vscode.commands.registerCommand(
    "bddScenarioRunner.generateAllureReport",
    async () => {
      runReportByType("allure");
    },
  );
  ...
  const runHeadlessProfile = testController.createRunProfile(
    ...
    (request, token) =>
      runFromTestItems(request, token, {
        askReport: true,
        runMode: "headless",
...
async function generateReportCommand(): Promise<void> {
    ...
    [
      {
        ...
        value: "playwright" as ReportType,
        description: "Open HTML report generated by Playwright",
      },
      {
        label: "Allure Report",
        value: "allure" as ReportType,
        description: "Generate and open Allure report",
      },
    ],
    {
      placeHolder: "Choose report type to generate/open",
  ...
  runReportByType(picked.value);
}

function runReportByType(reportType: ReportType): void {
  const command = buildReportCommand(reportType);
  const workspaceFolder = vscode.workspace.workspaceFolders?.[0]?.uri;
  runCommandInTerminal(command, workspaceFolder);

  const reportLabel = reportType === "playwright" ? "Playwright Report" : "Allure Report";
  vscode.window.showInformationMessage(`Generating ${reportLabel}.`);
}
...
async function runFromTestItems(
  ...
  options: TestingRunOptions,
): Promise<void> {
  const runMode = options.runMode;

  let selectedReportType: ReportType | null | undefined = options.defaultReportType;
  if (options.askReport) {
    selectedReportType = await resolveTestingReportType();
  ...
  const shouldGenerateReport =
    executedAnyScenario && !!selectedReportType && !token.isCancellationRequested && !stopAllTestItemsRequested;
  if (shouldGenerateReport && selectedReportType) {
    const reportLabel = selectedReportType === "playwright" ? "Playwright Report" : "Allure Report";
    run.appendOutput(`\r\n[INFO] Run completed. Generating ${reportLabel}...\r\n`);
  }
  ...
  if (shouldGenerateReport && selectedReportType) {
    runReportByType(selectedReportType);
  }
}

async function resolveTestingReportType(): Promise<ReportType | null | undefined> {
  const picked = await vscode.window.showQuickPick(
    [
      {
        label: "Do not generate report",
        value: "none" as const,
        description: "Finish run without opening any report",
      },
      {
      ...
      },
      {
        ...
        value: "allure" as const,
        description: "Generate and open Allure report",
      },
    ],
    {
      placeHolder: "Select post-run report option",
      title: "Playwright BDD Runner",
    },
  );
  ...
  return picked.value === "none" ? null : picked.value;
}
